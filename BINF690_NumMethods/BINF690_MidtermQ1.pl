...
###										NOTES											###
###########################################################################################
#In order to run the code, search for ###UPDATE and update the fx and f'x equations
...
print "Which method to use for solving the roots?\n";
...
print "3) 10*cos(x)*e**(-x/2)-x**(1/3)+1.8 (Midterm)\n";
print "Ans: ";
...
sub NR{
	...
	$Xi_new=sprintf("%.5f",$Xi_new);
	
	print "The root for this x is $Xi_new\n";
}	
...
sub f_solve{
	...
	} else{$fx=	10*exp(-$x/2)*cos($x)-($x**(1/3))+1.8; ###UPDATE NEW EQUATION
	}
...
sub f1_solve{
	...
	} else{$f1_fx=-10*exp(-$x/2)*sin($x)-5*exp(-$x/2)*cos($x)-(1/(3*$x**(2/3)))	; 		###UPDATE NEW EQUATION
	}
	return $f1_fx;
}
