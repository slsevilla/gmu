...
for (meta in colnames(sample_data(phy_obj))[1:5]){
...
#rarefy the data
phy_r = rarefy_even_depth(phy_obj, rngseed=1, sample.size=5000, replace=F) 
...
library(dplyr)
#merge to diet
phy_health = merge_samples(phy_r,"status")
phy_health

#select top OTUS
TopNOTUs <- names(sort(taxa_sums(phy_health), TRUE)[1:18])
phy_pr <- prune_taxa(TopNOTUs,phy_health)
...
p = plot_ordination(phy_r,ord,color="status",shape="ibd") + 
  geom_point(size=5)
p
file_name = "ordination_ibd"
ggsave(paste(analysis_dir,file_name,".jpg",sep=""),plot=p)

p = plot_ordination(phy_r,ord,color="status",shape="mental_illness_type_bipolar_disorder") + 
  geom_point(size=5)
p
file_name = "ordination_bip"
ggsave(paste(analysis_dir,file_name,".jpg",sep=""),plot=p)

p = plot_ordination(phy_r,ord,color="SampleType",shape="mental_illness_type_depression") + 
  geom_point(size=5)
p
...
#For Diet
adonis2(t(otu_table(phy_obj)) ~ status, 
        data=data.frame(sample_data(phy_obj)), permutations=99,
        method = "bray")

dist <- vegdist(t(otu_table(phy_obj)))
anova(betadisper(dist, data.frame(sample_data(phy_obj))$status))
...
#diet
mb_sub = mb_abund[rownames(mb_abund) %in% subset(sig_complete,val=="health")$OTU,]
TaxAnalysis(mb_sub,"status")
