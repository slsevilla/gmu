...
system_opt = "m"
#read in config
config_file = read_yaml(paste0(git_base,"aim3/analysis/config.yaml"))
#set dirs
...
#create new cols for status
metadata$mental_status=metadata$mental_illness
metadata$mental_status=gsub("No","healthy",metadata$mental_status)
metadata$mental_status=gsub("Yes","afflicted",metadata$mental_status)
metadata[is.na(metadata)]="No"
metadata$health_subtype=paste(metadata$mental_illness_type_bipolar_disorder,
                              metadata$mental_illness_type_depression,
                              metadata$mental_illness_type_ptsd_posttraumatic_stress_disorder,
                              metadata$mental_illness_type_schizophrenia,
                              metadata$mental_illness_type_substance_abuse)
unique(metadata$health_subtype)
metadata$health_subtype=gsub("No No No No No","healthy",metadata$health_subtype)
metadata$health_subtype=gsub("No Yes No No No","depression",metadata$health_subtype)
metadata$health_subtype=gsub("No Yes Yes No No","depression, PTSD",metadata$health_subtype)
metadata$health_subtype=gsub("Yes Yes Yes No No","bipolar, depression,\nPTSD",metadata$health_subtype)
metadata$health_subtype=gsub("No No Yes No No","PTSD",metadata$health_subtype)
metadata$health_subtype=gsub("No Yes No No Yes","depression,\nsubstance abuse",metadata$health_subtype)
metadata$health_subtype=gsub("No Yes No Yes No","depression, schizophrenia",metadata$health_subtype)
metadata$health_subtype=gsub("No No Yes Yes Yes","PTSD, schizophrenia,\nsubstance abuse",metadata$health_subtype)
unique(metadata$health_subtype)
metadata$ibd_status=metadata$ibd
metadata$ibd_status=gsub("I do not have this condition","healthy",metadata$ibd_status)
metadata$ibd_status=gsub("Diagnosed by prof","afflicted",metadata$ibd_status)
prep_data<-function(){
  #Read OTUS
  otus<-read_qza(paste0(data_dir,"04_filtered/4_tab.qza"))
  
  #Read taxonomy reference file
  taxonomy<-read_qza(paste0(data_dir,"05_class/scikit_silva.qza"))
...
count_samples<-function(df.in){
  #summarize mental health
  stats = c("healthy","afflicted")
  for (val1 in stats){
    count = nrow(subset(df.in,mental_status==val1))
  ...
  #summarize bmi, age, sex
  for (val1 in stats){
    for (val2 in sex){
      for (val3 in bmi){
        for (val4 in age){
          count = nrow(subset(df.in,mental_status==val1 & sex == val2 & bmi_cat == val3 & age_cat == val4))
  ...
  #summarize subtype
  for (val1 in unique(df.in$health_subtype)){
    for (val2 in sex){
      print(paste(val1,val2,nrow(subset(df.in,(health_subtype == val1) & (sex == val2))))) 
    }
  }
  ...
  #binomial tests
  #health
  print("bionmial test for health status")
  print(binom.test(57,55+57)$p.value)
  print(binom.test(55,55+57)$p.value)
  
  #by health and by
  #sex f/u, total u, f/h divided by total healthy
  nrow(subset(df.in,sex=="female" & mental_status=="afflicted"))
  nrow(subset(df.in,sex=="female" & mental_status=="healthy"))
  print("bionomial test for sex and status")
  print(binom.test(nrow(subset(df.in,sex=="female" & status=="afflicted")),
             nrow(subset(df.in,mental_status=="afflicted")), 
             nrow(subset(df.in,sex=="female" & mental_status=="healthy")) / 
               nrow(subset(df.in,mental_status=="healthy")),alternative="g")$p.value)
  
  #create df
  tmp = df.in[,c("age_cat","mental_status","bmi_cat")]
  ...
  #age
  mean(subset(tmp,mental_status=="healthy" & sex=="female")$age)
  mean(subset(tmp,mental_status=="healthy" & sex=="male")$age)
  mean(subset(tmp,mental_status=="afflicted" & sex=="female")$age)
  mean(subset(tmp,mental_status=="afflicted" & sex=="male")$age)
  print("test for age by status, female")
  print(t.test(formula = age ~ mental_status,
         data = tmp,
         subset = sex == "female"))
  print("test for age by status, male")
  print(t.test(formula = age ~ mental_status,
         data = tmp,
         subset = sex == "male"))
  
  #bmi
  mean(subset(tmp,mental_status=="healthy" & sex=="female")$bmi)
  mean(subset(tmp,mental_status=="healthy" & sex=="male")$bmi)
  mean(subset(tmp,mental_status=="afflicted" & sex=="female")$bmi)
  mean(subset(tmp,mental_status=="afflicted" & sex=="male")$bmi)
  print("test for bmi by status, female")
  print(t.test(formula = bmi ~ mental_status,
         data = tmp,
         subset = sex == "female"))
  print("test for bmi by status, male")
  print(t.test(formula = bmi ~ mental_status,
         data = tmp,
         subset = sex == "male"))
}
...
plot_read_depth<-function(phy.in){
  ...
  for(typeid in unique(sample_data(phy.in)$mental_status)){
  
    #create list of ids for ext.kit
    sample_list = rownames(subset(sample_data(phy.in),mental_status==typeid))
    
    for (sampleid in sample_list){
      sampledf[nrow(sampledf)+1,"mental_status"] = typeid
      sampledf[nrow(sampledf),"ibd_status"] = as.character(sample_data(phy.in)[sampleid,"ibd_status"])
  ...
  sumdf = summarySE(sampledf, measurevar="reads", groupvars=c("mental_status"))
  print(sumdf)
  
  #plot
  p1 = ggplot(sumdf, aes(x=mental_status, y=reads, fill=mental_status)) + 
        ...
        scale_fill_hue(name="mental_status") + # Legend label, use darker colors
        ggtitle(paste0("By Mental Status")) +
        theme_bw()
  print(p1)
    
  sumdf = summarySE(sampledf, measurevar="reads", groupvars=c("ibd_status"))
  print(sumdf)
  
  #plot
  p2 = ggplot(sumdf, aes(x=ibd_status, y=reads, fill=ibd_status)) + 
          geom_bar(position=position_dodge(), stat="identity",
                 colour="black", # Use black outlines,
                 size=.3) +      # Thinner lines
        geom_errorbar(aes(ymin=reads-se, ymax=reads+se),
        ...
        xlab("IBD Status") +
        ylab("") +
        scale_fill_hue(name="ibd_status") + # Legend label, use darker colors
        ggtitle(paste0("By IBD Status")) +
        theme_bw()
  print(p2)
    
  file_name = paste0(analysis_dir,"readlength_ibd_mental.png")
  pf = ggarrange(p1, p2,
            labels = c("A", "B"),
            ncol = 2, nrow = 1)
...
top_abun_plots<-function(phy.in,type.in){
  ...
  p = ggplot(phy_tmp, aes(x = Sample, y = Abundance, fill = Phylum)) + 
      theme(legend.position = "bottom", legend.key.width = unit(0.2,"cm"),
            ...
            ggtitle("Relative Abundance of Phylum: By Mental Health Status") +
            labs(fill = "Phylum")
  p1 = addSmallLegend(p)
  print(p1)
  ...
  p = ggplot(phy_tmp, aes(x = mental_status, y = Abundance, color=health_subtype)) + 
      theme(legend.position = "bottom", legend.key.width = unit(0.2,"cm"),
            axis.text.x = element_text(angle = 90, hjust = 1), 
            legend.text = element_text(size = 8))  +
            geom_boxplot() +
            xlab("Mental Status") +
            ylab("Abundnance") +
            ggtitle("Abundance of Top 50 OTU's\nPhylum: By Mental Sub-type, Mental Status") +
            labs(fill = "mental_status")+
...
top_abun_plots(merge_samples(phy_t1,"mental_status"),"Mental Health Status")
sig_test_abun<-function(phy.in,tax.in){
  ...
  prepare_df<-function(type.in){
    df_otu <- subset(phy_tmp, mental_status == type.in)
    #merge to phylum
  ...
  control_otu = prepare_df("healthy")
  cf_otu = prepare_df("afflicted")
...
create_heatmap_mental_status<-function(phy.in,tax.in){
  #melt OTUS  
  phy_tmp <- phy.in %>%
      tax_glom(taxrank = tax.in) # agglomerate at tax 
  
  tmp_df = data.frame(sample_data(phy_tmp))
  tmp_df = tmp_df[order(tmp_df$health_subtype),]
  
  #plot heatmap
  tmp_df = data.frame(sample_data(phy_tmp))
  sample_list = rownames(tmp_df[order(tmp_df$health_subtype),])
  
  tmp_df = data.frame(tax_table(phy_tmp))
  taxa_list = rownames(tmp_df[order(tmp_df$Phylum,decreasing=TRUE),])
  p1= plot_heatmap(phy_tmp,
                   "NMDS", "bray", "health_subtype", tax.in, low="#66CCFF", high="#000033",
                   na.value="white", sample.order=sample_list, taxa.order = taxa_list) + 
    ggtitle("Heatplot by Health Status and subtype") + 
    facet_grid(~health_subtype,scales="free")+
  ...
  #merge heatmaps
  file_name = paste0(analysis_dir,"heatplot_mental_status_",tax.in,".png")
  print(p1)
  ggsave(file_name,p1)
}
create_heatmap_mental_status(phy_t1,"Phylum")
```
...
richness_variables<-function(phy.in){
  ...
  for (rowid in rownames(df_ob)){
    ...
    compare_sampletype[nrow(compare_sampletype)+1,"type"] = as.character(sample_data(phy.in)[lookupval,"mental_status"])
    compare_sampletype[nrow(compare_sampletype),"health_subtype"] = as.character(sample_data(phy.in)[lookupval,"health_subtype"])
    compare_sampletype[nrow(compare_sampletype),"mental_status"] = as.character(sample_data(phy.in)[lookupval,"mental_status"])
  ...
  for(typeid in sort(unique(compare_sampletype$type))){
    for (subid in sort(unique(compare_sampletype$health_subtype))){
      sub_df = subset(compare_sampletype, type==typeid & health_subtype==subid)
      sampledf[nrow(sampledf)+1,"Sample Type"] = typeid
      sampledf[nrow(sampledf),"health_subtype"] = subid
  ...
  #add afflicted row 
  sub_df = subset(compare_sampletype, type=="afflicted")
  sampledf[nrow(sampledf)+1,"Sample Type"] = "afflicted"
  sampledf[nrow(sampledf),"health_subtype"] = "afflicted"
  sampledf[nrow(sampledf),"Inverse Simpson"] = paste0(round(mean(sub_df$INV),2)," (", round(sd(sub_df$INV),2),")")
  sampledf[nrow(sampledf),"Chao1"] = paste0(round(mean(sub_df$CH),2)," (", round(sd(sub_df$CH),2),")")
  sampledf[nrow(sampledf),"Observed Species"] = paste0(round(mean(sub_df$OBS),2)," (", round(sd(sub_df$OBS),2),")")
      
  sampledf= subset(sampledf,`Inverse Simpson` != "NaN (NA)")
  sampledf$health_subtype=gsub("\n"," ",sampledf$health_subtype)
  print(sampledf)
...
richness_plots <- function(phy.in){
  ...
  ## mental_status
  anova_result <- aov(alpha ~ mental_status, sample_df)
  print(summary(anova_result))
  tukey_result <- HSD.test(anova_result, "mental_status", group = TRUE)
  print(tukey_result)
  group_data <- tukey_result$groups[order(rownames(tukey_result$groups)),]
  p1 = ggplot(sample_df, aes(x = mental_status, y = alpha)) +
    geom_text(data = data.frame(),
              aes(x = rownames(group_data), y = max(sample_df$alpha) + 1, label = group_data$groups),
              col = 'black',
              size = 10) +
    geom_boxplot() +
    ggtitle("mental_status") +
    xlab("") +
    ylab("Alpha diversity index")
  ggsave(paste0(paste0(analysis_dir,"alpha_diversity_mental_status.png")),p1)
  
  ## health_subtype
  anova_result <- aov(alpha ~ health_subtype, sample_df)
  print(summary(anova_result))
  tukey_result <- HSD.test(anova_result, "health_subtype", group = TRUE)
  print(tukey_result)
  group_data <- tukey_result$groups[order(rownames(tukey_result$groups)),]
  p2 = ggplot(sample_df, aes(x = health_subtype, y = alpha)) +
    geom_text(data = data.frame(),
              aes(x = rownames(group_data), y = max(sample_df$alpha) + 1, label = group_data$groups),
              col = 'black',
              size = 10) +
    ...
    geom_boxplot() +
    ggtitle("health_subtype") +
    xlab("") +
    ylab("")
    ggsave(paste0(paste0(analysis_dir,"alpha_diversity_sub.png")),p2)
  
  ##IBD
  anova_result <- aov(alpha ~ ibd_status, sample_df)
  print(summary(anova_result))
  tukey_result <- HSD.test(anova_result, "ibd_status", group = TRUE)
  print(tukey_result)
  group_data <- tukey_result$groups[order(rownames(tukey_result$groups)),]
  p3 = ggplot(sample_df, aes(x = ibd_status, y = alpha)) +
    geom_text(data = data.frame(),
              aes(x = rownames(group_data), y = max(sample_df$alpha) + 1, label = group_data$groups),
              col = 'black',
              size = 10) +
    ...
    xlab("") +
    ylab("Alpha diversity index")
    ggsave(paste0(paste0(analysis_dir,"alpha_diversity_ibd.png")),p3)
  
  file_name = paste0(analysis_dir,"alpha_diversity.png")
  p = ggarrange(ggarrange(p1,p3,ncol=1,nrow=2,labels = c("A", "B","C")), p2,
            labels = c("","C"),
            ncol = 2, nrow = 1)
  ...
  p1 = plot_richness(phy_t1,measures =c("Observed","Simpson","Chao1"),
                     color="health_subtype",
                     x="mental_status")
...
create_ord_plots <-function(){
  ...
  p1 = plot_ordination(phy_t1, ordination,  
                       color="mental_status", title="Mental Health Status") + theme_bw() + 
    stat_ellipse(geom = "polygon", type="norm", alpha=0.4)
  
  p2 = plot_ordination(phy_t1, ordination,  
                       color="health_subtype", title="Mental Health Sub-type")
...
create_atich_plots<-function(phy.in,compare.in){
  ...
  #ADONIS test
  print(vegan::adonis(clr_dist_matrix ~ phyloseq::sample_data(phy.in)$mental_status))
  
  #Dispersion test and plot
  dispr <- vegan::betadisper(clr_dist_matrix, phyloseq::sample_data(phy.in)$mental_status)
...
create_atich_plots(phy_t1,"mental_health")
#https://microbiome.github.io/tutorials/PERMANOVA.html
#For mental_status
adonis2(t(otu_table(phy_t1)) ~ mental_status, 
        data=data.frame(sample_data(phy_t1)), permutations=99,
        method = "bray")
anova(betadisper(vegdist(t(otu_table(phy_t1))), data.frame(sample_data(phy_t1))$mental_status))
#For health_subtypes
adonis2(t(otu_table(phy_t1)) ~ health_subtype, 
        data=data.frame(sample_data(phy_t1)), permutations=99,
        method = "bray")
anova(betadisper(vegdist(t(otu_table(phy_t1))), data.frame(sample_data(phy_t1))$health_subtype))
```
...
cal_wilcox<-function(phy.in,tax.in){
  ...
  ps_wilcox$mental_status <- phyloseq::sample_data(phy_tmp)$mental_status
  
  #Define functions to pass to map
  wilcox_model <- function(df){
    wilcox.test(abund ~ mental_status, data = df)
  }
  wilcox_pval <- function(df){
    wilcox.test(abund ~ mental_status, data = df)$p.value
  }
  #Create nested data frames by OTU and loop over each using map 
  wilcox_results <- ps_wilcox %>%
    gather(key = OTU, value = abund, -mental_status) %>%
    group_by(OTU) %>%
    nest() %>%
...
cal_wilcox(subset_samples(phy_t1,health_subtype=="healthy" | mental_illness_type_depression=="Yes"),"Phylum")
cal_wilcox(subset_samples(phy_t1,health_subtype=="healthy" | mental_illness_type_ptsd_posttraumatic_stress_disorder=="Yes"),"Phylum")
#aldex2
cal_alex<-function(phy.in,tax.in){
  #Run ALDEx2
  aldex2_da <- ALDEx2::aldex(data.frame(phyloseq::otu_table(phy.in)), 
                             phyloseq::sample_data(phy.in)$mental_status, test="t", effect = TRUE, denom="all")
  
  file_name = paste0(analysis_dir,"aldex_mental_status.png")
  jpeg(file_name)
  par(mfrow=c(1,2),oma=c(1.5,2,3,1))    # set the plotting area into a 1*2 array
  ...
  mtext("ALDEx2 stratified by mental_status", line=0, side=3, outer=TRUE, cex=1.5)
  dev.off()
...
run_analysis<-function(phy.in){
  ...
  #mental_status
  sigtab = DiffAnalysis(phy.in,phyloseq_to_deseq2(phy.in, ~ mental_status), "healthy to afflicted","diffanal_mental_status")
  sig_complete = rbind(sig_complete,SigTables(sigtab,"mental_status"))
  
  #health_subtypes
  phy_tmp = subset_samples(phy_t1,health_subtype=="healthy" | mental_illness_type_depression=="Yes")
  sigtab = DiffAnalysis(phy_tmp, phyloseq_to_deseq2(phy_tmp, ~ mental_status), "healthy to depression","diffanal_healthy_dep")
  sig_complete = rbind(sig_complete,SigTables(sigtab,"HtoD"))
  
  phy_tmp = subset_samples(phy_t1,health_subtype=="healthy" | mental_illness_type_ptsd_posttraumatic_stress_disorder=="Yes")
  sigtab = DiffAnalysis(phy_tmp, phyloseq_to_deseq2(phy_tmp, ~ health_subtype), "healthy to PTSD","diffanal_healthy_PTSD")
  sig_complete = rbind(sig_complete,SigTables(sigtab,"HtoP"))
...
cal_rms<-function(phy.in,sub.id){
  ...
  clr_pcs <- data.frame(
    ...
    "pc2" = ord_clr$CA$u[,2],
    "pc3" = ord_clr$CA$u[,3],
    "Status" = phyloseq::sample_data(phy.in)$mental_status
  )
  ...
  pf = annotate_figure(pf, 
                       top = text_grob(paste0("RMS Prediction of afflicted\n",sub.id), face = "bold", size = 14))
  print(pf)
  ggsave(file_name,pf)
  ...
  file_name = paste0(analysis_dir,"rms_mental_status",sub.id,".png")
  jpeg(file_name)
  par(mfrow=c(1,1),oma=c(1.5,2,3,1))    # set the plotting area into a 1*2 array
  plot(cal)
  mtext(paste0("Actual Probability compared to Predicted Probability for afflicted\n",sub.id))
  dev.off()
  
}
cal_rms(phy_t1,"allsamples")
cal_rms(subset_samples(phy_t1,health_subtype=="healthy" | mental_illness_type_depression=="Yes"),"sub_dep")
cal_rms(subset_samples(phy_t1,health_subtype=="healthy" | mental_illness_type_ptsd_posttraumatic_stress_disorder=="Yes"),"sub_ptsd")
#sebal
#https://www.nicholas-ollberding.com/post/introduction-to-the-statistical-analysis-of-microbiome-data-in-r/
...
cal_sebal<-function(phy.in,compare.in,tax.in){
  ...
  #Run selbal
  cv_sebal <- selbal.cv(x = phy_df[1:(ncol(phy_df)-1)], 
                                y = as.factor(phyloseq::sample_data(phy_tmp)$mental_status), 
                             n.fold = 5, n.iter = 1, logit.acc = "AUC") 
  ...
  mtext("Prediction Results using Sebal\n All samples for mental_status", line=0, side=3, outer=TRUE, cex=1.5)
  dev.off()
...
cal_sebal(phy_t1,"mental_status","Phylum")
```
...
#download ref
tmp <- tempdir()
download_ref(tmp,reference='silva_ko')
run_tax4fun<-function(phy.in,tax.in){
  #create otu df
  phy_df = data.frame(t(data.frame(phyloseq::otu_table(phy.in))))
  phy_df = phy_df[,colSums(phy_df[,1:(ncol(phy_df))]) > 1000]
  tax_df = data.frame(tax_table(phy.in))
  tax_df = tax_df[ , colSums(is.na(tax_df)) == 0]
    
  #run analysis
  t4f_output <- themetagenomics::t4f(phy_df,rows_are_taxa=FALSE,
                   tax_table=data.frame(tax_table(phy.in)),
                   reference_path=tmp,type='uproc',short=TRUE,
                   cn_normalize=TRUE,sample_normalize=TRUE,drop=TRUE)
 
  #top ko
  top_ko = names(sort(colMeans(t4f_output$fxn_table),decreasing = TRUE)[1:15])
  ...
  #repeat analysis
  CLEAN <- prepare_data(otu_table=data.frame(t(otu_table(phy.in))),
                      rows_are_taxa=FALSE,
                      tax_table=data.frame(tax_table(phy.in)),
                      metadata=data.frame(sample_data(phy.in)),
                      formula= ~mental_status,
                      refs="healthy",
                      cn_normalize=FALSE,
                      drop=TRUE)
  TOPICS <- find_topics(CLEAN,K=5)
  TOPIC_EFFECTS<- est(TOPICS)
  FUNCTIONS <- predict(TOPICS,
                       reference='silva_ko',
                       reference_path=tmp,
                       cn_normalize=TRUE,
