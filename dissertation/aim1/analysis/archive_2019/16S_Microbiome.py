...
manifest.columns = manifest.columns.str.replace(' ', '')  # temporary - remove once cleaning is implemented in the pipeline
...
m = manifest.drop(columns=['ExternalID','SourcePCRPlate','Project-ID','ExtractionBatchID'],errors='ignore')
...
df_depth_no_blanks = df_depth[~df_depth.Sample_name.str.contains('|'.join(search_values ))]
plt.figure(dpi=100)
sns.set(style="whitegrid")
...
df_stats.join(manifest[['ExternalID']])[['ExternalID','input','filtered','denoised','merged','non-chimeric']].sort_values(['non-chimeric']).head(30)
...
manifest_no_blanks = manifest[~manifest.index.str.contains('|'.join(['Water','NTC']))]
...
df_rarify = pd.DataFrame(list(zip(values, samples, features, ids)),columns=['Sampling_depth','Percent_retained_samples','Percent_retained_seqs','Samples_excluded'])
df_rarify = df_rarify.set_index('Sampling_depth')
pd.set_option('display.max_colwidth', 0)
df_rarify[['Samples_excluded','Percent_retained_samples']]


# In[ ]:


df_rarify_tidy = df_rarify.reset_index().drop(columns='Samples_excluded').melt(id_vars='Sampling_depth')
...
for f in files:
    ...
    # should probably save this file, or even better, include in original manifest prior to analysis....
    cols = df.columns.drop([b,'depth','iteration','SourcePCRPlate','ExternalID','ExtractionBatchID'],errors='ignore')
    for c in cols:
        plt.figure(dpi=130)
...
plt.show()
# need to implement a fix, either here to deal with column header problems that are perpetuated throughout many analysis steps, or upstream in the pipeline.
